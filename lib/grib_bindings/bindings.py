...
if lib is None:
    for lib in ('/opt/ecmwf/eccodes/lib/libeccodes.so', '/usr/local/lib/libeccodes.dylib'):
...
grib_write_message = dll.grib_write_message
grib_write_message.restype = c_int
grib_write_message.argtypes = (grib_handle_p, c_char_p, c_char_p)
grib_write_message = convert_strings(grib_write_message)
grib_write_message = checked_return_code(grib_write_message)

####################################################################


grib_set_long = dll.grib_set_long
grib_set_long.restype = c_int
grib_set_long.argtypes = (grib_handle_p, c_char_p, c_long)
grib_set_long = convert_strings(grib_set_long)

grib_set_long = checked_return_code(grib_set_long)

grib_set_double_array = dll.grib_set_double_array
grib_set_double_array.restype = c_int
grib_set_double_array.argtypes = (grib_handle_p, c_char_p, c_double_p, c_size_t)
grib_set_double_array = convert_strings(grib_set_double_array)
grib_set_double_array = checked_return_code(grib_set_double_array)

grib_set_double = dll.grib_set_double
grib_set_double.restype = c_int
grib_set_double.argtypes = (grib_handle_p, c_char_p, c_double)
grib_set_double = convert_strings(grib_set_double)

grib_set_double = checked_return_code(grib_set_double)


def grib_set(handle, name, value):
    if isinstance(value, np.ndarray):
        array_p = value.ctypes.data_as(c_double_p)
        grib_set_double_array(handle, name, array_p, value.size)
    else:
        if int(value) == value:
            grib_set_long(handle, name, int(value))
        else:
            grib_set_double(handle, name, float(value))
